...
public class TestXmlFormatterTest {
	...
	public void makeFileName() throws Exception {
		...
		assertEquals("20090413152143_1_2_3_4.xml",
	...
	public void testParseFiles() throws IOException {
		String fileContent = getContentFromResource("files\\TestXmlFormatterTest\\testParseDirectories.xml");
		TestXmlFormatter testXmlFormatterTest = new TestXmlFormatter();
		Collection<JSONObject> result = testXmlFormatterTest.getAttachmentFiles(fileContent);
		assertEquals(1, result.size());
		JSONObject jsonObject = (JSONObject) result.toArray()[0];
		assertEquals("file.log", jsonObject.get("name"));
		assertEquals("text/plain", jsonObject.get("mimeType"));
	}
	@Test
	public void testParseDir() throws IOException {
		String fileContent = getContentFromResource("files\\TestXmlFormatterTest\\testParseDirectories.xml");
		TestXmlFormatter testXmlFormatterTest = new TestXmlFormatter();
		JSONObject result = testXmlFormatterTest.getAttachmentDirs(fileContent, "TestXmlFormatterTest");
		assertNotNull(result);
		assertEquals("TestXmlFormatterTest.zip", result.get("name"));
		assertEquals("application/zip", result.get("mimeType"));
	}

	// convert InputStream to String
	private String getContentFromResource(String fileName) throws IOException {
